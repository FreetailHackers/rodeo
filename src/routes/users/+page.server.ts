	...
	bulk: async (event) => {
		...
		if (action === 'admissions') {
			...
			if (role === 'OOS') {
				...
				for (const id of ids)
					await trpc(event).users.updateAdminTags({ userId: id, tag: 'isUT', value: true });
				return 'Tagged as UT!';
			}
			...
			if (role === 'OOS') {
				...
				for (const id of ids)
					await trpc(event).users.updateAdminTags({ userId: id, tag: 'isUT', value: false });
				return 'Removed UT tag!';
			}
