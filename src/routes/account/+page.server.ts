...
export const load = async ({ locals }) => {
	...
	let mealGroup = await trpc(locals.auth).users.getGroup();

	if (user.roles.includes('HACKER') && user.status.includes('CONFIRMED') && !mealGroup) {
		mealGroup = await trpc(locals.auth).users.setGroup();
	}
