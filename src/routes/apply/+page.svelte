	...
	<div class="inside-main-content">
		...
		<div id="status">
			...
			{:else if data.user.authUser.status === 'APPLIED'}
				<h1 style="color: #333A73">SUBMITTED</h1>
			...
			{:else if data.user.authUser.status === 'ACCEPTED'}
				<h1 style="color: #008B58">{data.user.authUser.status}</h1>
		...
		{#if data.user.authUser.status === 'CREATED'}
			...
			{:else}
				<p>
					Sorry, applications have closed. If space permits, you may sign up as a walk-in at the
					doors the day of the event, but we cannot make any guarantees.
				</p>
			{/if}
		{/if}
	</div>
