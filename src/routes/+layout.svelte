...
<nav>
	...
	<menu id="menu" bind:this={menu}>
		...
		{#if data.user !== null && (!data.user.role.includes('HACKER') || data.user.status === 'CONFIRMED')}
			<li><a href="/id">My Hacker ID</a></li>
		{/if}
		{#if data.user?.role.includes('ORGANIZER') || data.user?.role.includes('ADMIN')}
			<li><a href="/scan">Scan</a></li>
		{/if}
		{#if data.user?.role.includes('HACKER')}
			<li><a href="/apply">Apply</a></li>
		{:else if data.user?.role.includes('ADMIN')}
